...
<div class="page-header">
    <h1 class="page-title">{{ __('Professores') }}</h1>
    <div>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="javascript:void(0)">Dashboard</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ __('Professores') }}</li>
        </ol>
    </div>
</div>
<!-- PAGE-HEADER END -->
    ...
    <div class="col">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h3 class="card-title">{{ __('Listagem Professores') }}</h3>
                <a href="{{ route('teachers.create') }}" class="btn btn-primary">{{ __('Cadastrar') }}</a>
            </div>
            <div class="card-body">
            {{-- Alert Messages --}}
                            ...
                            <tr>
                                ...
                                <td class="td-actions text-right">
                                    <form action="{{ route('teachers.destroy', $teacher->id) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <a class="btn btn-primary" href="{{ route('teachers.show', $teacher->id) }}" data-original-title="" title="">
                                            <i class="material-icons">Ver</i>
                                            <div class="ripple-container"></div>
                                        </a>
                                        <a class="btn btn-warning" href="{{ route('teachers.edit', $teacher->id) }}" data-original-title="" title="">
