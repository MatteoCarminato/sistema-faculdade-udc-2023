    ...
    <div class="col">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h3 class="card-title">{{ __('Listagem Turma') }}</h3>
                <a href="{{ route('groups.create') }}" class="btn btn-primary">{{ __('Cadastrar') }}</a>
            </div>
            <div class="card-body">
            {{-- Alert Messages --}}
            ...
            <form method="GET" class="mb-5">
                ...
                <div class="table-responsive">
                    <table class="table border text-nowrap text-md-nowrap table-bordered mb-0" id="tableFormaPagamento">
                        ...
                        <tbody>
                            @foreach($groups as $local)
                            <tr>
                                <td>{{ $local->name }}</td>
                                <td class="td-actions text-right">
                                    <form action="{{ route('groups.destroy', $local->id) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <a class="btn btn-primary" href="{{ route('groups.show', $local->id) }}" data-original-title="" title="">
                                            <i class="material-icons">Ver</i>
                                            <div class="ripple-container"></div>
                                        </a>
                                        <a class="btn btn-warning" href="{{ route('groups.edit', $local->id) }}" data-original-title="" title="">
